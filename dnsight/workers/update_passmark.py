...
Для CPU: обновляет скоры для моделей из таблицы models.
Для GPU: использует raw_value атрибута "Графический процессор", получает скор,
        затем обновляет модель (model_id) каждого продукта, имеющего такое raw_value,
        и пересчитывает Benefit.
"""
...
def get_unique_gpu_raw_values(db: Session, attr_id: int) -> list:
    """Возвращает список уникальных raw_value для атрибута."""
...
def get_model_ids_for_raw_value(db: Session, attr_id: int, raw_value: str) -> list:
    """Возвращает список model_id продуктов, у которых есть attribute_value с данным raw_value."""
...
def update_model_scores_and_benefit(db: Session, model_ids: list, score: float) -> None:
    """Обновляет скор для всех переданных model_id и пересчитывает Benefit для их продуктов."""
...
def update_passmark_scores(headless: bool = True) -> None:
    logger.info("Запуск обновления PassMark")
    db = get_db()
    parser = PassMarkParser(headless=headless)

    try:
        # --- Обновление CPU (из таблицы models) ---
        ...
        if gpu_attr_id:
            ...
            for idx, raw_val in enumerate(raw_values):
                ...
                # --- ВСЕГДА ОБНОВЛЯЕМ, НЕ ПРОВЕРЯЕМ СВЕЖЕСТЬ ---
