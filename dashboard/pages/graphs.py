...
def plot_product_history(db: Session, component_type: str):
    ...
    # Для MB подпись скора другая
    score_title = "PassMark Score (расчётный)" if component_type == "Motherboard" else "PassMark Score"
    ...
    with col2:
        if not df_score.empty:
            fig = px.line(df_score, x="date", y="score", title=score_title, labels={"score": "баллы"})
            st.plotly_chart(fig, width='stretch', height=600)
        else:
...
def render(db: Session):
    ...
    for tab, pt in zip(tabs, types_with_products):
        with tab:
            plot_product_history(db, pt.name)